import { phoneIdentity, publicAuthConfig } from "./phone-auth.mjs";
...
// Verified phone identities are mapped to existing record IDs by the operator.
export default {
  async fetch(request, env, ctx) {
    const url = new URL(request.url);
    const origin = request.headers.get("Origin");
    ...
    if (!env.PHONE_USERS || !env.FIREBASE_PROJECT_ID || !env.ACCESS_LIMITER || !env.AI_LIMITER) return fail("利用設定を準備中です。運営者へお問い合わせください。", 503);
    try {
      ...
      if (url.pathname === "/api/auth-config" && request.method === "GET") {
        ...
        return config ? json(config, 200, headers) : fail("電話ログインは準備中です。", 503);
      }
      const token = (request.headers.get("Authorization") || "").match(/^Bearer ([A-Za-z0-9_.-]{1,8192})$/)?.[1];
      if (!token) return fail("設定から電話番号でログインしてください。", 401);
      const user = await phoneIdentity(token, env);

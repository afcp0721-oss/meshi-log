import {jwks, tokens, project, phoneUsers} from './phone-fixture.mjs';
import test from 'node:test';
import assert from 'node:assert/strict';
import { readFile } from 'node:fs/promises';
import { normalizeMealReport, normalizeXDraft, profileEntry } from '../analysis.mjs';
const source = (await readFile(new URL('../worker.js', import.meta.url), 'utf8'))
  .replace('"./phone-auth.mjs"', JSON.stringify(new URL('../phone-auth.mjs', import.meta.url).href))
...
function setup(t, options = {}) {
  ...
  const statements = [];
  const jobs = [];
  const env = { PHONE_USERS: phoneUsers, FIREBASE_PROJECT_ID:project, ACCESS_LIMITER:{async limit(){return {success:true}}}, AI_LIMITER:{async limit(){return {success:true}}}, GEMINI_API_KEY: 'test-key', DISCORD_WEBHOOK_URL: 'https://discord.com/api/webhooks/test', DB: {
...
test('phone gate blocks unauthenticated and mismatched identities before side effects', async t => {
...
test('missing security bindings and rate-limit outages fail closed', async t=>{
 ...
 for(const key of ['PHONE_USERS','FIREBASE_PROJECT_ID','ACCESS_LIMITER','AI_LIMITER']) assert.equal((await worker.fetch(req(),{...s.env,[key]:undefined},s.ctx)).status,503);
...
test('expired approval is denied and session returns only canonical identity',async t=>{
 ...
 const users=JSON.parse(s.env.PHONE_USERS);users.alice.expiresAt='2000-01-01';s.env.PHONE_USERS=JSON.stringify(users);

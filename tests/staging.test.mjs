...
const source = (await readFile(new URL('../staging/worker.mjs', import.meta.url), 'utf8'))
 ...
 .replace("import phoneScript from '../phone-auth.js';", 'const phoneScript = '+JSON.stringify(await readFile(new URL('../phone-auth.js', import.meta.url), 'utf8'))+';')
...
test('every staging route requires correct credentials', async () => {
 for (const path of ['/', '/app.js', '/phone-auth.js', '/api/auth-config', '/api/logs', '/api/image']) {

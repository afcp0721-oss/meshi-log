...
export async function signedToken(overrides = {}, headers = {}) {
  ...
  return new SignJWT({sub:'alice', aud:project, iss:'https://securetoken.google.com/'+project,
    iat:now, exp:now+3600, auth_time:now, email:'alice@example.test', email_verified:true, firebase:{sign_in_provider:'password'}, ...overrides})
...
export const emailUsers = JSON.stringify(Object.fromEntries(['alice','bob'].map(id=>[id,{userId:id,expiresAt:'2099-01-01',validAfter:0}])));

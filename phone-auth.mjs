...
let keysExpire = 0;
let pendingKeys;
function denied(message = '設定から電話番号でログインしてください。', status = 401) {
...
export async function verifyPhoneToken(token, projectId) {
  ...
  if (typeof payload.sub !== 'string' || !payload.sub || payload.sub.length > 128 ||
      ...
      typeof payload.phone_number !== 'string' || !/^\+[1-9]\d{7,14}$/.test(payload.phone_number)) throw denied();
  return payload;
}
export async function phoneIdentity(token, env) {
  const claims = await verifyPhoneToken(token, env.FIREBASE_PROJECT_ID);
  const users = JSON.parse(env.PHONE_USERS);
  const user = Object.hasOwn(users, claims.sub) ? users[claims.sub] : null;
  // IDs are assigned by the operator. Never attach records using client-supplied IDs or phone numbers.
  if (!user || user.disabled || typeof user.userId !== 'string' || !/^[A-Za-z0-9_-]{1,100}$/.test(user.userId) ||
    ...
    throw denied('電話番号を確認しました。利用開始には運営者の承認が必要です。', 403);
  }
